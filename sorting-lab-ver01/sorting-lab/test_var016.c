...
void quickSort(float arr[], int lowBound, int highBound){
    int partitions;
    (lowBound < highBound) && (partitions = partition(arr, lowBound, highBound));
    (lowBound < highBound) && (quickSort(arr, lowBound, partitions - 1), NULL);
    (lowBound < highBound) && (quickSort(arr, partitions + 1, highBound), NULL);

    //This section was removed in hopes of speeding up the algorithm.
